...
**SC-003 High escalation requires demonstrated wrongness, not just chain membership.** Being in the CE chain is necessary but not sufficient for High. SC-003 escalates to High when ALL of the following hold: (1) the cell is confirmed in the direct CE chain via FORMULA mode; AND (2) the value is demonstrably wrong — it contradicts a GiveWell standard, is arithmetically incorrect, or has been confirmed (via WebFetch or FORMULA inspection) to be a transcription error, wrong subgroup, or wrong methodology. When condition (2) cannot be met — because the finding is a documentation gap, a plausibility concern, or a researcher approach that is debatable but possibly defensible — file at **Medium**, even when the cell is confirmed in the CE chain. Examples where CE-chain membership alone does NOT justify High: a discount rate choice where the correct rate is debated, a funging scope choice that may be intentional, a formula that deviates from a template but has a plausible alternative justification. The threshold for High is: a researcher presented with this finding would agree it is wrong without needing to make a new judgment call — the error is self-evident from the data, formula, or standard.

**Exception for GBD vintage staleness**: When the stale GBD vintage cell is confirmed in the direct CE chain (≥2 FORMULA-mode hops to the CE output), SC-008 applies and mandates High — regardless of whether a value contradiction has been proved. SC-003 does not apply to GBD vintage findings when SC-008's chain-confirmation condition is met — SC-008 is the governing rule for that specific case.
...
| Funging and leverage adjustment cell formula errors (broken lookups returning 0, sign errors within the adjustment formula, misconfigured lookup keys) | `leverage-funging` | `ce-chain-trace`: when encountering a formula-level error in a funging or leverage adjustment cell (e.g., a VLOOKUP returning 0 due to a mismatched key), note the observation in reasoning and file a Low/Assumption SC-010 placeholder: "Possible issue — deferred to leverage-funging: funging adjustment formula at [cell] contains a lookup that may return 0 due to a mismatched key; leverage-funging owns the full finding." If leverage-funging is not in scope, ce-chain-trace should file as High/Formula [Wrong reference]. |
| Funging adjustment absent from direct CE chain (FN-008: funging not found in any formula referencing the CE output) | `leverage-funging` (Check 0) | `ce-chain-trace`: when funging is absent from the CE chain, file a Low/Assumption SC-010 placeholder and defer to leverage-funging Check 0. If leverage-funging is not in scope, ce-chain-trace should file as Medium/Adjustment per FN-008. |
| Wrong subgroup from source paper (SC-004) — WebFetch verification and full severity determination | `formula-check-data` (Check 5 — full WebFetch + subgroup cohort logic) | `source-data-check`: when a source tab cell note cites a study that may report multiple subgroup estimates, note the observation in reasoning and file a Low/Assumption SC-010 placeholder: "SC-004 subgroup check deferred to formula-check-data — source tab citation noted at [cell]." Do not independently WebFetch or file High/D findings for wrong-subgroup errors — formula-check-data owns the WebFetch and final severity determination. |
...
**Even when a rationale note already exists in the cell, the finding must still be filed at High/D.** The existing note does not resolve the finding — quote the note verbatim in the Explanation so the researcher can confirm the rationale is still current and intentional. An existing note changes the Recommended Fix wording (see M-2 note in key-params-check.md) but does not lower severity or justify skipping the finding.
