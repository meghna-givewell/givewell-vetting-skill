...
- **Estimated CE Impact** (H): Always begin with one of these standard phrases, then append a magnitude note if known:
  ...
  **Column H completeness by Error Type** (Pattern D — "never blank" applies to ALL severity levels when a determination can be made):
  ...
  - **Legibility findings at Low severity — column H must be blank**: Do not write any phrase (not even `No CE impact`) in column H for Low+Legibility findings. These findings route to Publication Readiness via blank column D (FORM-6); any populated column H value — especially a directional phrase — triggers the compaction Priority 2 routing rule and causes the finding to land in Findings instead of PR. The correct state for a Low+Legibility finding is: column D blank, column H blank. Legibility findings at Medium or High severity (which stay in Findings) follow the Inconsistency rule — blank is acceptable.
