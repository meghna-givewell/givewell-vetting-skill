...
- **Estimated CE Impact** (H): Always begin with one of these standard phrases, then append a magnitude note if known:
  ...
  **Column H completeness by Error Type** (Pattern D — "never blank" applies to ALL severity levels when a determination can be made):
  ...
  - **Summary**: For Formula, Parameter, and Adjustment — never blank at High, Medium, or Low. For Assumption — never blank at High or when direction is evident. For Inconsistency — blank is acceptable at all severities. For Legibility — see rule below.

  - **Legibility findings at any severity — column H must be blank**: Do not write any phrase (not even `No CE impact`) in column H for Legibility findings at any severity. For Low+Legibility findings, this is required because they route to Publication Readiness via blank column D (FORM-6); any populated column H value — especially a directional phrase — triggers the compaction Priority 2 routing rule and causes the finding to land in Findings instead of PR. The correct state for a Low+Legibility finding is: column D blank, column H blank. For Legibility findings at Medium or High severity (which stay in Findings), blank column H is also correct — Legibility errors do not produce a directional CE effect.

  **`No CE impact` must be written explicitly — never leave blank when the determination is zero**: When you have assessed a finding's CE impact and determined it is zero, write `No CE impact` in column H — do not leave column H blank. Blank means "CE impact not yet assessed"; `No CE impact` means "assessed and confirmed as zero." A blank column H on a Formula, Parameter, or Adjustment finding will be treated as an unassessed impact during validation and routing. This applies at all severity levels — write `No CE impact` explicitly even for Low findings when CE impact is confirmed zero. **Exception: Legibility findings — never write `No CE impact` (or any phrase) in column H for Legibility findings, regardless of severity.** The blank-column-H rule for Legibility (above) takes precedence.
