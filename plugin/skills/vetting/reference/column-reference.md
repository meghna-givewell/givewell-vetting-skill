...
| F | Explanation | 1–2 sentences max. Lead with the specific problem. Make a specific, falsifiable claim; include the actual value or formula. When citing a cell address, include the row label — the plain-English parameter name from column A of that sheet — alongside the cell reference (e.g., "malaria mortality rate (B14) = 0.87 but cross-reference in Main CEA (C22) = 0.79"). Plain language. Do not hedge what you can confirm. No chain traces. |
