---
name: vetting
description: "Run a full GiveWell-style spreadsheet vet on a workbook. Use when the user wants to vet a CEA or optionality BOTEC — checks formulas, sources, readability, hardcoded values, and severity-classified findings. Outputs a Findings Google Sheet with Dashboard, Findings, Hardcoded Values, and Confidentiality Flags tabs. Note: a separate Vetting Summary Google Doc is an optional output not currently produced automatically; the Dashboard tab serves as the inline summary."
argument-hint: "<Google Sheets URL or local file path>"
---

# /vetting — GiveWell Spreadsheet Vetter

**Skill version**: 2026-06-20 (v1.9.0) — update before each vet to get current agent calibrations. Standalone install: `git pull --rebase origin main` from `~/.claude/skills/vetting`. Plugin install: `/plugin marketplace update givewell-skills`.
...
- Lines beginning `HARDCODED|` → append to hardcoded values collection (6 agent-written fields: Sheet|Cell|Category|Value|Description|SourceURL, plus 2 Wave 1.5 fields added during verification: Verified? and Auto-check evidence — total 8 columns. The agent writes only the first 6; Wave 1.5 source-citation-verify adds columns 7 and 8 via HARDCODED_VERIFIED lines. Field 6 (SourceURL) is used by Wave 1.5 for WebFetch verification.)
...
**TA grant classification hint**: When Step 0.5 program orientation or the researcher's responses suggest the grant may involve technical assistance or capacity-building (e.g., the grant description or sheet title mentions "TA," "government capacity," "policy adoption," "program adoption," "speed-up," "technical support," "scale-up," "government adoption," or "government program"), set `is_ta_botec: true` in session context and pass it explicitly to ce-chain-trace-ta — the agent exits cleanly if no TA content is found after running, so false positives cost only time. Do not rely solely on tab naming conventions to determine TA status; researcher confirmation in Step 0.5 is authoritative. If the researcher's description is ambiguous (e.g., mentions "supporting government implementation," "supporting government scale-up," or any government partnership language without explicit TA terminology), ask: "Does this grant involve technical assistance activities that affect government program adoption rather than direct beneficiary delivery? If so, I'll run the TA-specific chain checks."

**TA recording gate**: If you ask the clarifying question and the researcher confirms this is a TA grant, immediately before proceeding to output setup or spawning any agent: (1) set `is_ta_botec: true` in session context; (2) identify the counterfactual burden tab name from the workbook's tab list (look for tabs named "Counterfactual Burden," "CF Burden," "Counterfactual Prevalence," "Burden Projection," "Government Adoption Model," "Adoption Model," or any tab containing "Adoption"); and (3) record both `is_ta_botec: true` and the counterfactual burden tab name in session context. **Do not proceed past this point without recording `is_ta_botec` — agent spawning must not begin until TA classification is fully set.**
...
Collect results before beginning Step 1. Add a brief "Literature context" paragraph to the program context summary passed to sub-agents, noting: (a) typical effect size ranges from external literature, (b) GiveWell's published assumptions for this intervention if found, and (c) any grantee-reported delivery data (coverage rates, costs per beneficiary). The plausibility agent uses this as a calibration anchor — e.g., "GiveWell's intervention report uses 12% mortality reduction; the model uses 9%, which is lower than GiveWell's own published estimate." **Fallback**: if all 4 searches return empty or irrelevant results (no usable effect size ranges, no GiveWell intervention page found, no grantee data), write in place of the literature paragraph: "External calibration unavailable — literature searches returned no usable results." Do not fabricate plausibility ranges or pass an unsubstantiated literature paragraph to sub-agents.

**Grant document financial trend extraction** (when a grant document is provided): After reading the grant document, extract and record in the program context summary any multi-year financial or operational data it contains — specifically: grantee revenue or budget trajectories, historical growth rates, coverage or reach trends, and figures the document uses to justify model inputs. Pass this as a "Financial trends from grant doc" note alongside the literature context. Heads-up and formula-check agents should check: do any model input values contradict a trend clearly visible in the grant document's own data? Example: if the grant document shows budget data with the most recent year at $X and growing, but the model uses a lower flat figure, flag as Low/H asking the researcher to confirm the flat assumption is intentionally conservative.
...
- **Values labeled "guess" or "best guess" are not findings.** A researcher labeling a cell "guess" or "best guess" is documenting uncertainty transparently — this is acceptable modeling practice and does not require a Findings or Publication Readiness entry. Do not file `Parameter` or `Assumption` findings solely because a cell note contains "guess" or "best guess." The Hardcoded Values sheet captures these cells for researcher review. **Exception**: if a "best guess" value is also outside the plausible range for that parameter (e.g., a coverage rate of 1.5, a mortality reduction of 150%, or a cost per beneficiary far below any comparable program), file an `Assumption` finding at the appropriate severity — the label acknowledges uncertainty but does not excuse an implausible value.
...
- A154: `band1_end` | B154: the last row number of band 1 (e.g., `150`) — **write this only when `band_count > 1`**; when band-split is not active (`band_count = 1`), write `N/A (band_count=1)` in B154 so the gap-fill agent can distinguish "no banding" from "metadata missing." This value is read by the gap-fill agent (Step 10b) to run the cross-band root cause trace (Check 2.5) after context compaction.

---

**Create staging sheets — do this immediately after vet metadata, before spawning any agents.** Each Wave 1, Wave 2, and Reconcile agent writes its findings exclusively to its own dedicated staging tab, eliminating all concurrent write conflicts on the Findings sheet. Call `mcp__hardened-workspace__create_sheet` for each tab in the tables below — fire all creates in a parallel batch. After creation, write the 9-column header row to row 1 of each tab using `modify_sheet_values`: `Finding # | Sheet | Cell/Row | Severity | Error Type/Issue | Explanation | Recommended Fix | Estimated CE Impact | Status`.
...
- formula-check-data: `stg-data-C/D` (band 2), `stg-data-E/F` (band 3), `stg-data-G/H` (band 4); reconcile staging `stg-rec-data-b2/b3/b4`
- formula-check-edge-cases: `stg-edge-C/D` (band 2), `stg-edge-E/F` (band 3), `stg-edge-G/H` (band 4); reconcile staging `stg-rec-edge-b2/b3/b4`
- formula-check-structure: `stg-struct-C/D` (band 2), `stg-struct-E/F` (band 3), `stg-struct-G/H` (band 4); reconcile staging `stg-rec-struct-b2/b3/b4`

**Wave 2 banded agents**:
- sources: `stg-src-C/D` (band 2), `stg-src-E/F` (band 3), `stg-src-G/H` (band 4); reconcile staging `stg-rec-src-b2/b3/b4`
- heads-up-evidence: `stg-evid-C/D` (band 2), `stg-evid-E/F` (band 3), `stg-evid-G/H` (band 4); reconcile staging `stg-rec-evid-b2/b3/b4`
- heads-up-intervention: `stg-int-C/D` (band 2), `stg-int-E/F` (band 3), `stg-int-G/H` (band 4); reconcile staging `stg-rec-int-b2/b3/b4`
- readability: `stg-rdbl-C/D` (band 2), `stg-rdbl-E/F` (band 3), `stg-rdbl-G/H` (band 4); reconcile staging `stg-rec-rdbl-b2/b3/b4`
- heads-up-epi *(special case — section-split per band)*: `stg-epi-C` (Section A, band 2) and `stg-epi-D` (Section B, band 2); reconcile staging `stg-rec-epi-b2`; and similarly `stg-epi-E/F` + `stg-rec-epi-b3` for band 3, `stg-epi-G/H` + `stg-rec-epi-b4` for band 4

Create only the extra tabs for the bands that actually exist (band_count=2 → only k=2 tabs; band_count=3 → k=2 and k=3 tabs). Write header rows to all extra tabs immediately after creation, in the same `modify_sheet_values` batch as the main header rows.
...
>
> **Exceptions — three agents write directly to their output sheets, not to a staging tab**: (1) **hardcoded-values** writes directly to the Hardcoded Values sheet (all rows); (2) **source-citation-verify** writes directly to the Hardcoded Values sheet's columns G and H (Verified? and Auto-check evidence); (3) **sensitivity-scan** writes directly to the Confidentiality Flags sheet. These three agents have no staging tab. The staging-tab rule does not apply to them.
>
...
>
> **Explanation — length and style**: Column F is 3 sentences maximum; aim for 2. Write for a GiveWell researcher who may not have the source spreadsheet open — every explanation must stand alone. Include the row label — the plain-English parameter name from column A of that sheet — alongside every cell reference. **Legibility test**: before writing, confirm the explanation answers (a) what is wrong, (b) what the correct value or behavior should be, and (c) why it matters for the model.
>
> Per error type — **Parameter/Inconsistency**: state the current value and the correct value, e.g., "Malaria mortality rate (B14) = 0.87; GW parameter is 0.79, overstating CE." For Inconsistency, flag which tab is authoritative if known: "Coverage rate is 0.87 in Main CEA (B14) but 0.79 in Inputs (C22) — confirm which is authoritative." **Formula**: lead with the functional effect — what the formula currently computes incorrectly — then give the technical fix, e.g., "[Wrong reference] Program costs (E14) sums through a non-program row, inflating costs by ~12%; range should be B14:B21 not B14:B22." **Assumption/Adjustment**: state what is absent or misapplied, then the consequence.
>
> **High findings**: weave a brief consequence clause into the first sentence — "overstating CE by ~15%", "understating program costs" — not a separate sentence and not a verbatim copy of column H. Do not hedge what you can confirm — write "malaria mortality rate (B14) uses the wrong value" not "malaria mortality rate (B14) may be wrong." No chain traces, no reasoning.
>
> **Low-severity category grouping**: Before writing any Low findings, group all Low observations into these 7 standard categories and file one row per category per sheet: **Documentation gaps** (missing notes/rationale on non-key params) | **Formula robustness** (defensive programming gaps safe at current values) | **Stale annotations** (outdated labels/notes where values are correct) | **Optimistic assumptions** (favorable modeling choices without documented rationale) | **Minor rounding** (≤2% from source, <5% CE impact) | **Structural completeness** (missing optional tabs/sections) | **Minor inconsistencies** (cross-tab mismatches not affecting CE). Lead the Explanation with the category name and count: "Documentation gaps: 3 rows lack source notes — coverage rate (B14), coverage rate CIV (C14), vaccine efficacy (B22). Add a cell note for each." Exception: structurally unique issues that can only appear once (single missing required section, single discount rate omission) may be filed as a standalone row. See `reference/output-format.md` Grouping section for full definitions.
>
> **Publication Readiness Sourcing — one row per citation class per sheet, never per cell**: For all Sourcing findings in Publication Readiness, group by citation class and sheet. File one PR row per citation class per sheet listing all affected cells in column C. Do not file individual per-cell Sourcing findings. The exception "cells need different sources" does NOT justify filing separately — different cells needing different individual sources is normal; the recommended action (add a source note) is still the same. Only file separately when the recommended action genuinely differs per cell. A sheet with 10 unsourced cells → 1–2 grouped PR rows, not 10 rows.
>
...
4. **Band-split protocol** — evaluate before spawning:
   ...
   **Band computation**:
   - `band_size = 150`
   - `populated_rows` = the last-populated-row number of the **main CEA sheet only**, as identified in Step 2 (the row number of the last non-empty row returned by the FORMULA-mode read of that sheet; use the same value already recorded for the `split_row` calculation above)
...
**AGENT_COMPLETE location — per-agent reference table**: Parsers (Wave 2.5 silent failure check, Wave 3 self-verification, pre-Wave-1.5 guard) must handle all of the following variants:
...
Do not assume all agents write AGENT_COMPLETE to the same location or column. Always check the table above before parsing completion status.
...
**Pre-Wave-1.5 guard — check hardcoded-values agent completed**: Before spawning the source-citation-verify agent, read the Hardcoded Values sheet in batched increments (`'Hardcoded Values'!A1:H50`, `A51:H100`, continuing until two consecutive batches return no non-empty rows) and search for a row where column B = `hardcoded-values` AND column D = `AGENT_COMPLETE`. Do not rely on the last non-empty row — the AGENT_COMPLETE marker may appear before later rows written by source-citation-verify. If a row matching both conditions is found, the hardcoded-values agent completed normally. If no such row is found: announce `⚠️ hardcoded-values agent appears not to have completed — Hardcoded Values sheet may be incomplete or empty. Wave 1.5 source citation verification requires a populated Hardcoded Values sheet to run. Options: (1) re-run the hardcoded-values agent and then re-run Wave 1.5, or (2) skip Wave 1.5 by proceeding to Wave 2. Ask the researcher which to do before continuing.` Do not skip silently.
...
**TA BOTEC — counterfactual burden pair**: When `is_ta_botec: true` is set in session context (per Step 0.5), confirm the counterfactual burden tab name recorded in Step 0.5 (the TA recording gate requires this to be identified and recorded there — look for tabs named "Counterfactual Burden," "CF Burden," "Counterfactual Prevalence," "Burden Projection," "Government Adoption Model," "Adoption Model," or any tab containing "Adoption" if not already identified). Spawn two additional `heads-up-epi` instances (TA-A and TA-B) with that tab as the only vetted sheet in session context. Assign staging tabs: TA-A → `stg-epi-ta-A`, TA-B → `stg-epi-ta-B`. Append to the TA-A instance context: `Instance scope: heads-up-epi-TA-A — Section A (Epidemiological Parameter Checks) only on the TA burden tab. Skip Section B entirely.` Append to the TA-B instance context: `Instance scope: heads-up-epi-TA-B — Section B primary + adversarial Section A on the TA burden tab.` Pass to **both** TA-A and TA-B instances: "**Counterfactual burden tab focus**: You are auditing the counterfactual burden/prevalence tab only (`{tab name}`). Apply all TA-specific checks in your prompt. Use formula mode reads on every AVERAGE, OFFSET, or INDEX formula in the tab. Do not read other tabs except to verify cross-references." Pass to **TA-B instance only** (not TA-A): "In addition, pay particular attention to: (a) AVERAGE() range endpoints — verify they cover TA exit year + 5 years; (b) time series column headers — read them explicitly to confirm which year each column represents. These are Section B checks and are your primary scope." Apply the standard adversarial B-instance preamble to the TA-B instance only. If the workbook has no identifiable counterfactual burden tab, skip the TA-A/TA-B pair and note this in chat.
...
| Pair | Staging sheet A | Staging sheet B | Reconcile staging sheet |
|---|---|---|---|
| formula-check-data band k | `stg-data-[C/E/G]` | `stg-data-[D/F/H]` | `stg-rec-data-b[k]` |
| formula-check-edge-cases band k | `stg-edge-[C/E/G]` | `stg-edge-[D/F/H]` | `stg-rec-edge-b[k]` |
| formula-check-structure band k | `stg-struct-[C/E/G]` | `stg-struct-[D/F/H]` | `stg-rec-struct-b[k]` |
| sources band k | `stg-src-[C/E/G]` | `stg-src-[D/F/H]` | `stg-rec-src-b[k]` |
| heads-up-evidence band k | `stg-evid-[C/E/G]` | `stg-evid-[D/F/H]` | `stg-rec-evid-b[k]` |
| heads-up-intervention band k | `stg-int-[C/E/G]` | `stg-int-[D/F/H]` | `stg-rec-int-b[k]` |
| readability band k | `stg-rdbl-[C/E/G]` | `stg-rdbl-[D/F/H]` | `stg-rec-rdbl-b[k]` |
| heads-up-epi band k *(section-split)* | `stg-epi-[C/E/G]` (Section A) | `stg-epi-[D/F/H]` (Section B) | `stg-rec-epi-b[k]` |
...
**notes-scan completion check**: After all Wave 2.5 reconciliation agents complete, read staging sheets `stg-nscn-A` and `stg-nscn-B` separately in batched increments (`A1:I50`, `A51:I100`, continuing until two consecutive batches return no non-empty rows) — **the MCP tool returns at most 50 rows per call**; do not use a single large range. For each tab, check for an AGENT_COMPLETE marker independently:
...
3c. If ce-chain-trace-ta-A column F contains **neither** `TA grant signals confirmed:` nor `No TA grant signals found` (e.g., contains `tab-names=indeterminate` or any other phrase not matching either canonical form), announce:

> ⚠️ TA cross-check inconclusive: ce-chain-trace-ta-A AGENT_COMPLETE column F contains neither canonical TA phrase — found: "[column F text]". Cannot determine TA classification from this agent's output. Consider re-running ce-chain-trace-ta with `is_ta_botec` explicitly set based on researcher confirmation before treating Wave 3 findings as authoritative on TA scope.
