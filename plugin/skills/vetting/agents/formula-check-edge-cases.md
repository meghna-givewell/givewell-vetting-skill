...
- **LAMBDA()**: Verify that the number of parameters defined in the LAMBDA matches the number of arguments passed at every call site. A LAMBDA defined as `LAMBDA(x, y, x+y)` called with only one argument returns an empty result in some Google Sheets versions rather than a visible error. For each LAMBDA definition, count parameters and cross-check each call site. Flag as **Medium/H** (**Formula**) with Researcher judgment needed ✓ if a parameter/argument count mismatch is found in any CE-chain cell.

For display-only cells not in the CE chain, flag any of the above conditions as **Low/H**. For CE-chain cells, flag as **Medium/H** with Researcher judgment needed ✓.
