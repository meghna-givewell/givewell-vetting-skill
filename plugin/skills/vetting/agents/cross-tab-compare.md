...
- Findings sheet ID
- Staging sheet: write findings to your dedicated staging tab (name provided in session context; default: `stg-xcomp`) starting at row 2
- User email for MCP calls
- Program context from Step 0.5, including any declared-intentional deviations

**Self-detection — run this first**: Check the in-scope sheets list for both a Simple CEA tab (names containing `Simple CEA`, `Simple`, or `SimpleCEA`) AND a Main CEA or CEA tab (names containing `Main CEA`, `CEA`, or `BOTEC` — but not `Simple`). If no Simple CEA tab is found using those patterns, also try alternate names: `Cost Model`, `CE Analysis`, `CE Model`. If no Main CEA tab is found using those patterns, also try those same alternates for the main/full model tab. If after trying all alternates fewer than two matching tabs exist but at least one CEA-type tab was found: skip all checks except Check 12 (worldview/scenario weight sum-to-one, which requires only a single Main CEA tab) and proceed to Check 12. After completing Check 12, write the AGENT_COMPLETE marker with column F: `Partial run: Check 12 only (no Simple CEA tab found — cross-tab checks 1–11 skipped). Tabs scanned: [list tab names]. COVERAGE_ROWS: Main CEA all rows read. Filed [K] findings in rows 2–[K+1]. Staging sheet: stg-xcomp.`. If no CEA-type tab is found at all, write your completion marker with the reason in column F (e.g., `SKIP: No CEA tab found matching standard or alternate patterns. Tabs scanned: [list tab names].`) and stop. Do not silently skip — the reason must be visible in the AGENT_COMPLETE row.
...
Before running any check, read reference/pitfalls.md using the Read tool. Apply every entry relevant to cross-tab consistency, benchmark validation, and inconsistency findings — especially SC-001, SC-021, SC-028, FP-007, SC-017, SC-018, and the pre-filing mandatory checklist (SC-022 through SC-028).
...
After completing Check 5, perform a second benchmark check that is independent of cross-tab consistency. Before running this check, read `reference/key-parameters.md` using the Read tool and extract the canonical GiveDirectly benchmark mortality/effectiveness value from it — do not rely on any inline hardcoded value here. Both tabs may share the same stale benchmark — which makes them internally consistent but both wrong — so cross-tab agreement does not clear this check.
...
2. Compare it against the canonical value loaded from key-parameters.md. **No tolerance zone applies** — any deviation, however small, is a bright-line finding per SC-001 and the GW-standard-parameters bright-line rule in `reference/output-format.md`
3. If the value differs from the key-parameters.md canonical value in any way, file **High/Parameter**: "[Tab name] [cell] contains a GiveDirectly benchmark value of [value], which deviates from the canonical key-parameters.md value of [canonical value]. Both tabs may share this stale value, so cross-tab consistency does not resolve this. Either update the benchmark to the current GW value ([canonical value]) OR add a rationale note documenting why the older value is retained."
...
**Deferral note**: First determine whether the readability agent is in scope by checking session context for the active agent list. If readability IS in scope, do not file this finding independently — write a SC-010 Low/Assumption placeholder only ('Simple CEA section ordering — deferred to readability agent per pitfalls.md Cross-Agent Scope Reference'). If readability is NOT in scope, file Check 10 normally (no deferral, no placeholder). Readability owns Simple CEA section ordering when running.
