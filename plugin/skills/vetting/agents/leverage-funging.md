...
**Check 3b — SC-013 denominator ratio check**: Identify the full grant budget cell and the CE denominator cell. Compute the ratio: full_grant_budget / CE_denominator. If this ratio exceeds 1.2×, file **High/Parameter**: "Cost denominator at [cell] equals [denominator]; full grant budget is [budget] — ratio [ratio]×. Reported CE of [X]× falls to ~[X/ratio]× when the full budget is used as the denominator. Restrict the denominator exclusion to costs that genuinely do not generate grant benefits, or document the exclusion rationale in a cell note." If the ratio is ≤1.2×, file Medium. If a cell note explicitly justifies the exclusion (e.g., 'evaluation costs funded by a separate earmarked grant'), file Low. Cite SC-013 in reasoning. Do not file if the excluded costs genuinely do not generate the grant's benefits.
...
**Incremental coverage formula check**: For any row labeled "incremental," "net new," or equivalent (e.g., "incremental coverage," "net new beneficiaries"), read the cell formula in FORMULA mode. If the formula is a plain cell reference (e.g., `=B12` or `=DataTab!C5`) with no subtraction of a baseline or counterfactual value (i.e., no `-` operator subtracting another cell), file **High severity (column D), Error Type: Formula [Wrong reference]**: "Row '[label]' ([cell]) is labeled as incremental/net-new coverage but the formula is a plain reference `[formula]` referencing total coverage rather than a delta value. If this cell holds total program coverage rather than the GiveWell-funded increment, CE will be materially overstated. Verify whether the cell already returns a delta value from the source tab; if not, subtract the baseline coverage cell."

**Required Error Type**: Assumption (or Formula [Wrong reference] for the incremental coverage formula check above)
...
If such a literal is found, first check whether the literal value can be confirmed wrong (i.e., contradicts a parameter source or is demonstrably incorrect). If the literal value cannot be confirmed wrong from available context, add this cell to the structural-formula-quality list (SC-024) for one grouped PR/Legibility item rather than filing Medium/Parameter. Only file **Medium severity (column D), Error Type: Parameter** when the literal value is confirmed incorrect or has no verifiable source and the parameter is confirmed in the direct CE chain (SC-009): "Leverage formula at [cell] embeds the leverage ratio as a hardcoded literal [value] (e.g., `[formula snippet]`). The leverage ratio should reference a named parameter cell with a source note so that changes propagate and reviewers can verify the value. Add a dedicated parameter cell for this ratio and replace the literal with a cell reference."
...
Flag undocumented leverage parameters as follows: if the value is outside typical ranges for this intervention type, first run a FORMULA-mode trace from this parameter to the CE output — if ≥1 hop to the CE output is confirmed (SC-009 / SC-012), file **Medium/Assumption** and note chain membership in the Explanation; otherwise file **Low/Assumption**. If the value is plausible and the only issue is missing documentation, file as Error Type: Legibility (column E), column D blank (Publication Readiness routing). For Low/Assumption findings, write column H as follows: write "Direction unknown" when the direction of CE impact is unclear; write "Raises CE — magnitude unknown" or "Lowers CE — magnitude unknown" when the direction is known. Do not leave column H blank on Low/Assumption findings.
