...
File as **Medium/H** if the wrong-country concern is suspected but not confirmed (e.g., data values are plausibly consistent with the wrong country but not definitively verified): "[Tab name] may contain [wrong country] data — verify the source and add a note documenting the geography or proxy rationale if appropriate."
...
### Check G — SC-004/SC-005/SC-007 subgroup citation deferral and co-ownership

When reviewing source tab rows, you may notice cells that cite a study and use a subgroup estimate (e.g., a row label indicates "children under 5, rural" but the cited source reports multiple subgroup estimates for different age bands or geographies). Because SC-004 (wrong subgroup from source paper) requires WebFetch to verify and involves detailed cohort logic, **formula-check-data (Check 5) is the primary owner** of SC-004 subgroup determinations. Do not independently WebFetch the source to verify SC-004 subgroup correctness, and do not file High findings for SC-004 subgroup mismatches.

SC-005 (methodology mismatch) and SC-007 (value matching a study subgroup) are co-owned by source-data-check per pitfalls.md Applies-to lists. When a source tab row shows a methodology mismatch (SC-005), file directly at Medium minimum. When a value matches a specific study arm (SC-007), file directly at High with both interpretations noted.
