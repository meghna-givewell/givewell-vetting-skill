...
**Scope partition — VOI adjustment vs. leverage-funging**: This agent checks formula-level UoV reference correctness (Steps 6, 6b, 6d). VOI-only funging scope (SC-014 — funging formula references a VOI-adjusted subtotal rather than total-CE row) is owned by leverage-funging Check 0b. If a VOI-only funging scope issue is observed here, file a Low/Assumption SC-010 placeholder only (see Step 6c). Do not file an independent High/Adjustment finding for SC-014 from this agent.
...
SC-014 (funging applied to VOI component only) is owned by leverage-funging Check 0b. If a VOI-only funging scope issue is observed here, file a Low/Assumption SC-010 placeholder only, using this wording: "Possible issue — deferred to leverage-funging Check 0b: funging formula at [cell] references [VOI subtotal row label] rather than total-CE row; SC-014 scope check. See Cross-Agent Scope Reference in pitfalls.md." Do not file an independent High/Adjustment finding for this pattern.
