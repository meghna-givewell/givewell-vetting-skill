...
Read reference/pitfalls.md using the Read tool. Apply all SC-*, FP-*, and FN-* entries — including SC-003 (CE-chain High threshold), SC-008 (GBD vintage escalation), SC-017 (High count calibration), SC-018 (Low filing threshold), and FP-007 (no-CE-impact protection) — when classifying severity of any new finding filed in Checks 1–3, as well as entries relevant to cascade analysis, coverage gaps, and Won't Fix verification.
...
| Parameter value accuracy and grant amount consistency | `stg-params-A` and `stg-params-B` | Low/Assumption: "No finding or clean declaration for formula-check-parameters (covers grant amount consistency, stale-year, and wrong-country checks). Confirm both stg-params-A and stg-params-B contain AGENT_COMPLETE rows." |
...
| CE arithmetic replication (end-to-end CE check) | `stg-cerep-A` and `stg-cerep-B` | Low/Assumption: "No finding or clean declaration found for CE replication. Confirm ce-replication ran and completed Steps 1–7 — verify AGENT_COMPLETE column F contains a CE_stated and CE_replication value. A missing ce-replication run means the end-to-end CE arithmetic was not independently verified." |
| Open-ended suspicion check | `stg-susp` (single instance) | Low/Assumption: "No finding or clean declaration found for the suspicion-first holistic check. Confirm suspicion-first ran — verify stg-susp contains an AGENT_COMPLETE row with a suspicions-formed count. A missing suspicion-first run leaves the vet without its open-ended non-checklist pass." |
| Cross-tab CE and scope consistency | `stg-xcomp` (single instance) | Low/Assumption: "No finding or clean declaration found for cross-tab comparison. Confirm cross-tab-compare ran — verify stg-xcomp contains an AGENT_COMPLETE row. A missing cross-tab-compare run means the Simple CEA vs. Main CEA structural comparison and scope delta check were not performed." |
| Parameter value accuracy (stale data, wrong-country, SUMIFS staleness) | `stg-params-A` and `stg-params-B` | Low/Assumption: "No finding or clean declaration found for parameter value accuracy. Confirm formula-check-parameters A and B ran — verify both stg-params-A and stg-params-B contain AGENT_COMPLETE rows. A missing formula-check-parameters run leaves stale-year and wrong-country parameter checks uncovered." |
...
Coverage declaration: "Category coverage check complete. Categories confirmed covered: [N/13]. Gaps filed: [list of categories or 'none']. key-params-check coverage log: [N of M — complete / incomplete / staging tab not found]."

---

## Check 5 — Pass B completeness

Read `stg-pass-b` in batched increments (`stg-pass-b!A1:I50`, `stg-pass-b!A51:I100`, …) until two consecutive batches return no non-empty rows. Collect every row where column D = `AGENT_COMPLETE`. For each such row, record the agent label from column B.

The 14 expected Pass B agent labels are:
...
If stg-pass-b is entirely empty (no AGENT_COMPLETE rows found), file one grouped finding instead of 14 individual ones: Low/Assumption — "stg-pass-b contains no AGENT_COMPLETE rows — none of the 14 Pass B agents are confirmed. Pass B (second-opinion CE-focus pass) may not have run. Re-run Pass B or confirm with the researcher before treating the vet as complete."
...
Coverage declaration: "Pass B completeness check complete. AGENT_COMPLETE rows found: [N/14]. Missing agents: [list or 'none']. Findings filed: [K]."
...
Assign new Finding IDs continuing sequentially from the last ID assigned by compaction (e.g., if compaction assigned through F-018, write F-019, F-020, …). Insert new findings at the appropriate severity tier — after the last finding of that tier and before the next tier divider — using `modify_sheet_values`. Update the **tier-level** divider row count accordingly (e.g., if a new High finding is added, change `─── High (4 findings) ───` to `─── High (5 findings) ───`). Also update the matching **per-sheet sub-divider** for the finding's sheet within that tier: find the row whose column B matches `─── [Severity] — [Sheet name] ([K] findings) ───` for the relevant severity and sheet name, and change K to K+1. When searching for the tier-level divider to update, match only the exact format `─── [Severity] ([N] findings) ───` — do not confuse it with per-sheet sub-dividers, which contain a sheet name after the severity word.
...
**Divider-count re-read**: After all new gap-fill findings are written to the Findings sheet, re-read the first two columns (A:B) of the Findings sheet in full (batched 50-row increments as above). Count two types of divider rows separately — both are rows where column B contains `───` and column A is blank (or contains no F-* ID):
- **Tier-level dividers**: column B matches exactly `─── High (N findings) ───`, `─── Medium (N findings) ───`, or `─── Low (N findings) ───` (no sheet name after the severity word).
- **Per-sheet sub-dividers**: column B matches `─── [Severity] — [Sheet name] ([K] findings) ───` (has a sheet name after the severity word).

Verify the tier-level divider count is between 1 and 3 (one per active severity tier). If it is 0, flag: "No tier-level divider rows found — verify compaction wrote section dividers correctly." Per-sheet sub-dividers are written by compaction and may number anywhere from 0 to N_sheets — do not flag the total divider count exceeding 3; it is expected. Include the tier-level and per-sheet sub-divider counts separately in the final coverage declaration.
...
After all checks and new findings are written, write a completion row to the Findings sheet row immediately after the last finding row: column B = `final-review-gap-fill`, column D = `AGENT_COMPLETE`, column F = `COVERAGE_ROWS: Findings sheet rows 2–[last_row] | Output sheet: Findings (gap-fill writes directly — no staging tab). [K] gap-fill findings added in rows [first_new_row]–[last_new_row].`

The count phrase `[K] gap-fill findings added` must appear verbatim in column F — the final-review-validation agent's CROSS-3 check parses this exact phrase to compute the expected total finding count. Replace `[K]` with the actual integer count of findings written by gap-fill (excluding the AGENT_COMPLETE row itself). If gap-fill added 0 findings, write `0 gap-fill findings added`.
