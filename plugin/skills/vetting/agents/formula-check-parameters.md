...
For key disease burden cells that use SUMIFS or COUNTIFS to pull from large source data tabs (GBD, IHME, IGME), verify the formula result is consistent with a direct computation from the source data. Using the source tab data in the pre-read cache, independently compute the expected value by locating the relevant rows (matching year, geography, cause, and age group) and summing or averaging.

**Threshold**: If the independently computed value differs from the SUMIFS result by more than **0.5 percentage points** (not 1 — SUMIFS staleness can produce small absolute discrepancies even when material new rows have been added to the source data), flag as **Low/H** (**Formula**): "SUMIFS result appears stale — formula returns X but direct read of [source tab] gives Y. SUMIFS formulas do not always recalculate automatically when the source data is updated."

**Range endpoint coverage** (additional check, run for every SUMIFS/COUNTIFS formula): After independently computing the expected value, also verify that the formula's sum_range and criteria_range row endpoints match the last populated row in the source data tab. To check: read the last populated row of the source tab using `read_sheet_values` (a single 50-row batch starting from the expected end row will confirm the extent). If the SUMIFS range ends at row N but the source data tab has populated rows through row M > N, the formula silently excludes rows N+1 through M regardless of how small the current numeric discrepancy appears — this will compound as the tab grows. Flag as **Medium/H** (**Formula**): "[cell] SUMIFS range ends at row [N] but source data tab [tab name] has data through row [M] — [M−N] rows of data are excluded from this computation. Update the range to `sum_range!A1:A[M]` or convert to a FILTER-based formula that auto-expands with the data."
...
**Tab-name vintage scan (prerequisite to Check 3)**: Before scanning cell notes, read the full list of tab names in the workbook (from session context or the workbook structure read in Step 0). For each tab whose name contains "GBD," "IHME," or "IGME," check whether the tab name also contains a year that is ≥2 years before the model's grant period start year (e.g., a tab named "GBD Data 2021" in a 2025–2027 model). For each such tab, identify all cells in the vetted sheets that reference it via FORMULA mode using `read_sheet_values`. Treat each such cross-sheet reference as a GBD vintage trigger equivalent to a cell-note citation — proceed to the SC-008 FORMULA-mode trace for each such cell, even if the cell itself has no note. Include these tab-name-triggered checks in the stale-year note coverage declaration.
...
This rule applies regardless of whether the deviation is directionally conservative. A conservative deviation still misstates CE and should be documented. The key-params-check agent also applies this rule; the Wave 3 compaction agent will deduplicate overlapping findings across stg-params-A/stg-params-B and stg-kp-A/B. stg-params-A and stg-params-B are reconciled at Wave 2.5 via a dedicated formula-check-parameters reconcile agent (writes to stg-rec-params), in the same way as all other A/B pairs. Findings from stg-params-A and stg-params-B that overlap with key-params-check findings are further deduplicated at Wave 3 compaction.
...
**Tab prefix required**: When calling modify_sheet_values, prefix the range with your staging sheet name — e.g., stg-params-A!A2:I2 (use the exact staging sheet name from session context — it will be stg-params-A or stg-params-B). A range without a tab prefix writes to the active sheet, not your staging tab.

Column reference: **A** Finding # (leave blank) | **B** Sheet | **C** Cell/Row | **D** Severity (write one of: `High` | `Medium` | `Low` — use two-axis notation in reasoning only; the /D or /H suffix is not written to the sheet) | **E** Error Type/Issue (write the exact label only — no additional text; choose one of: Formula | Parameter | Adjustment | Assumption | Legibility | Inconsistency | Sourcing | Box Link) — for Sourcing and Box Link findings (publication-readiness), leave column D blank | **F** Explanation (3 sentences max, aim for 2; write for a researcher who may not have the spreadsheet open; include the row label (plain-English name from column A) alongside every cell; Parameter/Inconsistency: "currently X; correct value is Y"; Formula: functional effect first then technical fix; High findings: include a brief consequence clause; no chain traces) — when column E is Formula, begin column F with: [Copy-paste] | [Wrong reference] | [Year range] | [Sign error] | [Wrong operator] | [Off-by-one] (required by output-format.md) | **G** Recommended Fix (one sentence; lead with an imperative verb) | **H** Estimated CE Impact (exactly one of: Raises CE — [estimate] | Lowers CE — [estimate] | Raises CE — magnitude unknown | Lowers CE — magnitude unknown | No CE impact | Direction unknown) — use exact em-dash ( — ) with spaces on both sides; en-dash or plain hyphen variants will cause sort failures in the Wave 3 compaction agent | **I** Status (leave blank — reconcile agent writes WONT_FIX here; compaction strips column I when writing to the final Findings sheet)

**Publication-readiness findings** (Error Type: Sourcing or Box Link): write them to your staging sheet in the same 9-column format, with column D (Severity) left blank. The compaction agent routes them to Publication Readiness based on Error Type. Do not write directly to the Publication Readiness sheet. Legibility findings that carry an assigned severity (High/Medium/Low) are substantive findings — write column D normally and they stay in the main Findings sheet. Only Legibility findings with Low severity get column D left blank — do not write 'Low' in column D; the Low determination belongs in reasoning only.
