...
This rule applies regardless of whether the deviation is directionally conservative. A conservative deviation still misstates CE and should be documented. The key-params-check agent also applies this rule; the Wave 3 compaction agent will deduplicate overlapping findings across stg-params and stg-kp-A/B. stg-params IS reconciled, but at Wave 3 compaction — not at the Wave 2.5 reconcile step. The Wave 2.5 reconcile agent does not read stg-params; findings written here are deduplicated by final-review-compaction instead. This means stg-params duplicates are caught and resolved, just at a later stage than other staging sheets.
