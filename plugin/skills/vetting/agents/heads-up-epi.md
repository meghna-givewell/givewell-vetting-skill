...
If session context does not set an `is_ta_botec` flag or identify a counterfactual burden tab, skip the TA-A/TA-B scope and do not run these checks. When skipping, write a single AGENT_COMPLETE row to your staging sheet with: Column B: `heads-up-epi`; Column D: `AGENT_COMPLETE`; Column F: `COVERAGE_ROWS: none | Staging sheet: [name from session context]. Filed 0 findings. [reason for skip: no is_ta_botec flag / no counterfactual burden tab identified in session context].`
