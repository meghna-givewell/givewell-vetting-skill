...
- **Source citations using only a staff member's first name** — e.g., "per Jack's model," "Bea's analysis," "from Meghna's spreadsheet," "per CRA" — are not traceable publication-ready citations and must be replaced with a link or full document title before publication. Exception: when readability is also running, note instances in reasoning only; do not file a separate finding — readability's exhaustive source-column scan covers this.
...
**Instance B only — verify Instance A completion**: Before writing your AGENT_COMPLETE row, read the Instance A staging sheet. Derive the Instance A tab name from your own staging tab name by replacing the final `-B` suffix with `-A` (for example, if your staging tab is `stg-nscn-B`, the Instance A tab is `stg-nscn-A`). Do not hardcode `stg-nscn-A` — use the derived name so the check still works if staging tabs are renamed. Check whether the Instance A staging tab contains a row where column D = `AGENT_COMPLETE`. If no such row is found, include a warning in your AGENT_COMPLETE column F: `WARNING: Instance A staging tab has no AGENT_COMPLETE row — Instance A may have failed silently. Verify Instance A completed before final compaction.`
