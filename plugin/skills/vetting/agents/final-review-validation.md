...
1a. **Fix cell-type and units compatibility check**: After reading the cell formula in step 1, also read the same cell in FORMATTED_VALUE mode and in UNFORMATTED_VALUE mode. Determine: (a) whether the cell is a formula cell or a hardcoded literal; (b) what the cell's display format implies about expected units. Then inspect the Recommended Fix in column G:
   - **Percentage format mismatch**: if the cell displays as a percentage (e.g., `87.0%`) but stores a decimal (UNFORMATTED_VALUE = `0.87`), and the proposed fix specifies a whole number (e.g., `87`), append to the finding's Explanation: "Note: cell is percentage-formatted and stores a decimal (displays 87% but stores 0.87). The proposed fix value 87 would be interpreted as 8700% — the correct substitute value is 0.87." Do not mark the finding resolved.
   - **Formula-to-hardcoded substitution**: if the current cell is a formula cell but the proposed fix replaces it with a hardcoded literal, append to column F: "Note: this fix replaces a formula with a hardcoded value — confirm the replacement value will not become stale if upstream inputs change."
   - **Hardcoded-to-formula substitution**: no compatibility concern — proceed normally.
   - If no format or type mismatch is detected, proceed without appending any note.
