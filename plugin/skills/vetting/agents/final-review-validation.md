...
2. **Gap-fill count adjustment**: Before comparing counts, scan the rows collected in Step 1 for a row where column B = `final-review-gap-fill` AND column D = `AGENT_COMPLETE`. From column F of that row, extract the number of findings gap-fill added. The text reads: `[K] gap-fill findings added` or `[K] new findings added` or similar — look for a number followed by "finding" or "new finding." Record as `N_gap_fill` (default to 0 if the marker is absent or the count is not parseable).
