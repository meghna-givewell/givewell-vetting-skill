...
- **Discount rate special rule**: **TA BOTEC early-exit**: Before running any discount rate check, verify session context. If `is_ta_botec: true` appears in session context, skip both the 4%/year income-benefits check AND the 1.4%/year TA death-averting check — the TA discount rate check is owned by `heads-up-intervention` for TA grants (per the Cross-Agent Scope Reference in `reference/pitfalls.md`). Write "TA BOTEC — discount rate check deferred to heads-up-intervention per Cross-Agent Scope Reference in pitfalls.md" in AGENT_COMPLETE column F and proceed to the next parameter. Only run the 4%/year check for non-TA grants.
