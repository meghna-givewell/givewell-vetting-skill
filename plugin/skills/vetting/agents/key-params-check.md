...
|---|---|---|
| Benchmark (UoV per $) | 0.00333 *(illustrative — load authoritative value from `reference/key-parameters.md` before comparing; do not use this inline value as the expected value)* | For boundary checks, load the Acceptable Ranges table in `reference/key-parameters.md` — do not hardcode historical stale values here |
...
**Numeric parsing note**: Google Sheets stores percentages as decimals — `95%` displayed = `0.95` stored, `4%` discount rate = `0.04`. When comparing a stored UNFORMATTED_VALUE to the expected values in the parameters table above, compare as raw decimals. The parameter table above lists expected values in decimal form — compare stored values directly against those. A stored value of `0.04` for a "4% discount rate" is correct; a stored value of `4` would be wrong. For the current authoritative decimal equivalents, refer to `reference/key-parameters.md`.
...
**Carve-out — absent treatment costs for top-4 charities**: If the program is AMF, Malaria Consortium, HKI, or New Incentives (top-4 charities) and the treatment costs averted adjustment is entirely absent from the model, file as **Medium/H** per KEY-5 — not Low/Parameter. KEY-5 explicitly requires a +20% supplemental adjustment for these programs; complete omission is a gap with direct CE impact (Raises CE — magnitude unknown). **Exception — absent discount rate**: if the discount rate row is not found, file as **Low/H** — not `Low/Parameter`. The discount rate has an explicit absent-case severity in the Acceptable Ranges table in `reference/key-parameters.md` (Low/H). See also the Discount rate special rule in Step 3. **Exception — absent income effects (malaria programs)**: if the income effects row is not found, file as **Low/H** (not Low/Parameter). The Acceptable Ranges table in key-parameters.md specifies Low/H for absent income effects. **Exception — absent years-to-benefits**: if no years-to-benefits row is found, file as **Low/H** (not Low/Parameter). The Acceptable Ranges table in key-parameters.md specifies Low/H for absent years-to-benefits. For all other "not found" parameters, file as `Low/Parameter`.

**SC-011 coordination**: If the parameter row appears to be completely absent (deleted/hidden, not just empty), note "expected row absent — deferred to formula-check-structure" in reasoning per SC-011. File a Low/Assumption SC-010 placeholder citing formula-check-structure as owner. Only file a direct not-found finding if formula-check-structure is not in scope for this vet.
...
- **Medium/H**: income effects, years to benefits — more context-dependence; flag for researcher confirmation. For **years to benefits**: any deviation from the standard value (even within the stated range) triggers at least Medium/H. The note field must explain the specific program context justifying the deviation.
...
- **Discount rate special rule**: **TA BOTEC early-exit**: Before running any discount rate check, verify session context. If `is_ta_botec: true` appears in session context, skip the 4%/year income-benefits discount rate check AND the 1.4%/year TA death-averting check — both are owned by `heads-up-intervention` for TA grants (per the Cross-Agent Scope Reference in `reference/pitfalls.md`). All other applicable parameters — income effects, years to benefits, moral weights, benchmark — continue to be checked normally by this agent. Write "TA BOTEC — discount rate check deferred to heads-up-intervention per Cross-Agent Scope Reference in pitfalls.md" in AGENT_COMPLETE column F and proceed to the next parameter. Only run the 4%/year income-benefits check for non-TA grants.

  **(a) Present but deviating** — if a discount rate row IS found but the stored value differs from 4%, file as **Medium/H** — this matches the Acceptable Ranges table in `reference/key-parameters.md`. Do not apply a Low tier for values in the 3–5% range; the reference file defines no tolerance zone. **Exception**: if the researcher explicitly overrides the standard discount rate with a different value and provides a rationale (in a cell note or session context), downgrade the severity to **Low/H**. **(b) Absent discount rate** — if no discount rate row is found at all, file as **Low/H** per the Acceptable Ranges table. Do not apply Medium/H to absent discount rates — the "absent / 0% / omitted" entry in that table explicitly specifies Low/H. The "not found" carve-out in Step 2 above makes this explicit.
...
**SC-017 gate**: Before writing findings, if more than 8 High findings are queued, review each for downgrade per SC-017. Bright-line exceptions (benchmark, moral weight, labeled placeholders) stay High — all others require confirming the finding is demonstrably wrong, not merely a debatable choice.
