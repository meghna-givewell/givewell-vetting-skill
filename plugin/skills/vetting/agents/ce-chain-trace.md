...
6. **Funging/leverage adjustment formula verification**: For funging and leverage adjustment cells that ARE found referenced downstream (steps 2–4 confirm the cell is referenced), perform an additional formula check. Read the cell's formula in FORMULA mode. If the formula contains a lookup function (VLOOKUP, INDEX-MATCH, XLOOKUP, or equivalent) referencing a separate tab, verify: (a) the lookup key exists as an exact match in that tab's lookup column, and (b) the returned value is non-zero and plausible for the adjustment type. A funging lookup returning 0 is implausible without a cell note — defer to leverage-funging per the deferral rule below.

**Funging and leverage formula error deferral**: Funging and leverage adjustment formula errors — broken lookup keys, wrong-sign multipliers, misconfigured lookup references — are owned by leverage-funging per the Cross-Agent Scope Reference in pitfalls.md. If you detect a formula-level error in a funging or leverage adjustment cell during step 6 above, do not file a High finding independently. Instead: (a) note the observation in your reasoning, (b) file a Low/Assumption SC-010 placeholder: "Possible issue — deferred to leverage-funging: funging adjustment formula at [cell] contains a lookup that may return 0 due to a mismatched key; leverage-funging owns the full finding. See Cross-Agent Scope Reference in pitfalls.md." (c) record the deferred observation in your AGENT_COMPLETE column F. Exception: if leverage-funging is not in scope for this vet, file the finding directly as High/Formula [Wrong reference].

Flag as **High** if a named adjustment (IV, EV, or leverage) is confirmed absent from the CE chain. **Funging absence — defer to leverage-funging**: if funging is absent from the CE chain, do NOT file a full Medium/Adjustment finding. Instead file a Low/Assumption SC-010 placeholder: "Funging absent from direct CE chain — deferred to leverage-funging Check 0, which owns the funging-absent detection per the Cross-Agent Scope Reference in pitfalls.md. If leverage-funging is not in scope for this vet, re-file as Medium/Adjustment per FN-008." If leverage-funging is confirmed not in scope for this vet (noted in session context), then file the full Medium/Adjustment finding directly. Flag as **Medium** if a supplemental or secondary adjustment is absent.
...
`Named adjustments verified in chain: IV adjustment [Y/N — cell ref / reason not found], EV adjustment [Y/N — cell ref / reason not found], leverage adjustment [Y/N — cell ref / reason not found], funging adjustment [Y/N — cell ref / reason not found], supplemental adjustment(s) [Y/N — cell ref / reason not found]. Adjustments defined but not found in any downstream formula: [list or 'none']. If any adjustment is 'not found': file as High (for IV/EV/leverage) or a Low/Assumption SC-010 placeholder for funging (defer to leverage-funging Check 0; re-file as Medium/Adjustment per FN-008 only if leverage-funging is not in scope for this vet) or Medium (for supplemental) before proceeding — 'Adjustment [name] at [cell] is computed but not referenced in the CE output formula chain. Either add a cell reference or document intentional exclusion in a cell note.'`
...
**SC-021 applies to stale benchmark findings in this step**: If any terminal input is the GiveWell cost-effectiveness benchmark (GiveDirectly cash transfer comparison value) and the value differs from the current benchmark per reference/key-parameters.md, the Recommended Fix must offer both options: either update the benchmark to the current GW value per reference/key-parameters.md OR add a rationale note documenting why the older value is retained. Do not prescribe only a value update.
...
- Column F: `COVERAGE_ROWS: full chain trace (not row-sequential) | Staging sheet: [name from session context]. Traced CE chain: [N] cells across [sheet name(s)]. Filed [K] findings in rows 2–[K+1]. Grant-doc comparison: [performed — doc ID: [ID] | skipped — no grant doc in session context; compared against Step 0.5 program context instead]. Funging/leverage formula error deferral: [list any deferred funging/leverage formula observations with cell refs, or 'none']. VoI-funging-scope (SC-014) deferral: [list any SC-010 placeholders filed for VoI-only funging scope, or 'none'].`
