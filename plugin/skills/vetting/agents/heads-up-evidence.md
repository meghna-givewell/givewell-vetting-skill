...
Load CEA Consistency Guidance (`1aXV1V5tsemzcFiyx2xAna3coYAVzrjboXeghbe949Q8`) via `get_doc_content` at the start of every run — before reading any sheet data. This matches the load-at-start behavior of heads-up-epi and ensures consistent application of cross-cutting modeling methodology guidance across all parallel agents. Do not defer this load to a conditional trigger.
...
**Before starting any checks**: Read `reference/pitfalls.md` using the Read tool. Apply every entry relevant to evidence quality, CE chain confirmation, and missing-source severity (especially SC-003, SC-004, SC-005, SC-009, FP-002).
...
**Multi-CE models (FORM-27)**: When the model contains multiple CE outputs (e.g., a main scenario and one or more sensitivity scenarios), the top-5 selection must use CE values from the **main (non-sensitivity) scenario only**. Do not include sensitivity scenario CE outputs in the ranking — sensitivity scenarios exist to test assumptions, not to define the central estimate. If it is unclear which scenario is the main scenario, ask the researcher before proceeding.
...
**Count-based completion gate**: Before writing AGENT_COMPLETE, write the following summary line in your reasoning: "[N] hyperlinks found in cell notes, [M] fetched, [K] skipped (low-priority or non-fetchable)." Then evaluate: if any high-priority links remain unfetched due to context limits or tool failures — where high-priority means the link is in a row containing a key parameter (top-5 sensitivity list) or a confirmed direct CE-chain cell (within any number of hops of the final CE cell) — do not write AGENT_COMPLETE. Instead write AGENT_INCOMPLETE to your staging sheet (Column B: `heads-up-evidence`; Column D: `AGENT_INCOMPLETE`; Column F: `Hyperlink audit incomplete due to context limits. [N] hyperlinks found, [M] fetched, [K] skipped. The following high-priority links were not fetched: [list each cell ref and URL].`). Only write AGENT_COMPLETE when either (a) all high-priority links have been fetched or filed as inaccessible, or (b) no high-priority links exist among the unfetched set.
...
**Cross-source effect construction**: When the model's primary effect size (mortality reduction, morbidity reduction, coverage-to-outcome ratio) is derived by dividing or comparing values from two *different* studies or data sources — rather than taken from a randomized within-trial comparison — apply FN-007 from pitfalls.md. To identify a cross-source construction: read the formulas for the mortality-reduction or effect-size rows and check whether the numerator (treated rate) and denominator (baseline/control rate) each cite different studies or different rows with different source notes. **If a cross-source construction is identified, the WebSearch below is mandatory — not optional — before writing any finding or clearing this check.** Run WebSearch `"[intervention] [outcome] Cochrane review"` or `"[intervention] [outcome] meta-analysis systematic review"` and record the result. If a Cochrane review or comparable meta-analysis exists and reports a materially different raw effect size (>15% relative), file as **Medium/Assumption** citing FN-007. If no external meta-analysis exists, file as **Low/Assumption** requesting documentation that a within-trial comparison was considered. Record in your mandatory check log: "cross-source effect construction — [cross-source present / not present]; WebSearch result: [result summary or 'no cross-source identified, search skipped']." Do not fire this check when the effect size is taken directly from a single RCT or systematic review that reports a treated-vs-control comparison internally.
...
**IPTp dosing schedule check (FORM-28)**: When vetting an IPTp (Intermittent Preventive Treatment in pregnancy) model, check whether the model's assumed dosing schedule matches the current WHO IPTp dosing schedule. If the model hardcodes a specific number of doses or a schedule that does not match current WHO guidance, file as Medium/H (Parameter). Recommended fix language: "Update to current WHO IPTp dosing schedule." Do not reference a specific year in the recommended fix — the WHO schedule may have been updated since the file was written, and a year-specific fix could itself become stale.

**VAS indirect deaths ceiling check (FORM-30)**: When vetting a VAS (Vitamin A Supplementation) CE model that includes indirect deaths, after reading the indirect-deaths row and the total-deaths row, compute indirect deaths as a share of total deaths in the model. If indirect deaths exceed 50% of total deaths, flag as **Medium/H (Assumption)**: "The indirect-death proportion ([computed %]) exceeds 50% of total deaths in the model, which is unusually high. Verify this proportion against the current GiveWell VAS model and confirm the indirect-death estimate is correctly specified." This ceiling is a plausibility flag only — it does not imply the value is wrong, but it requires the researcher to confirm.
...
Evidence quality:
  ...
  cross-source effect construction [___ — cross-source present / not present; WebSearch result: ___]
