...
**Part B — Cross-Cutting CEA Parameters doc (all CEA types)**: consistency-check is the designated owner of Cross-Cutting CEA Parameters doc value comparisons. In a standard vet where both agents run, skip Part B entirely — do not read the doc and do not file findings from it. File Part B findings only if session context explicitly indicates consistency-check is not running for this vet.
