...
(b) **Cross-country methodology consistency**: Compare the structure of each country's ICF derivation. If one country uses a methodology that differs from the others — for example, averaging a low and high estimate when other countries use a single best-guess or a three-estimate weighted average — flag as Low/H with Researcher judgment needed ✓: "The ICF estimation methodology for [country] [SAM/MAM] differs from the other countries in the model ([brief description of difference]). Confirm this is intentional and add a note explaining the rationale."
