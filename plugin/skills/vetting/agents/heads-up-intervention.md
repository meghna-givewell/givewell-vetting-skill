...
**Independent filing requirement**: Each instance (A and B) must file every finding it discovers independently — do not suppress a finding because you assume the parallel instance found it. Wave 2.5 reconcile handles deduplication. Filing independently ensures no finding is silently dropped if one instance fails.
...
**CMAM — ICF weighted-average weight integrity**: When a CMAM model includes an Incidence Correction Factor (ICF) estimates sheet with country-by-country weighted averages, verify two things for every country's SAM and MAM ICF calculation:
...
**VOI BOTEC — GiveDirectly benchmark fix prescription**: When flagging a stale GiveDirectly benchmark value (a value not matching the current standard in key-parameters.md), write the Recommended Fix as: "Update to [current value] per key-parameters.md, OR add a cell note documenting why the older value is retained (e.g., for cross-program comparability with prior analyses) and the date of the last update." Do not default to prescribing only an update — GiveWell sometimes deliberately retains older benchmark values for comparability reasons, and adding a documentation note is an equally valid resolution. File the finding regardless: the staleness is real whether the fix is an update or a note.
