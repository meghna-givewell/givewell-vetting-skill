...
Write all findings to your staging sheet starting at row 2. Use the standard 10-column layout (columns A–J), leaving column D (Severity) and column H (Estimated CE Impact) blank — all findings from this agent are Publication Readiness items.

Column reference: **A** Finding # (leave blank) | **B** Sheet | **C** Cell/Row | **D** Severity (leave blank) | **E** Error Type/Issue (`Box Link` or `Sourcing`) | **F** Explanation (1–2 sentences; state what was found and why it needs review) | **G** Recommended Fix (one sentence: "Verify this [document / Box file] is publicly accessible before publishing. If it cannot be made public, replace with an external citation or remove the link.") | **H** Estimated CE Impact (leave blank) | **I** Researcher judgment needed (leave blank) | **J** Status (leave blank)
