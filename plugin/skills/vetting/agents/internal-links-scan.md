...
Column reference: **A** Finding # (leave blank) | **B** Sheet | **C** Cell/Row | **D** Severity (leave blank) | **E** Error Type/Issue (`Box Link` or `Sourcing`) | **F** Explanation (1–2 sentences; state what was found and why it needs review) | **G** Recommended Fix (one sentence: "Verify this [document / Box file] is publicly accessible before publishing. If it cannot be made public, replace with an external citation or remove the link.") | **H** Estimated CE Impact (leave blank) | **I** Status (leave blank)
