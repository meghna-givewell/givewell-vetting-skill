...
Read all rows from staging sheet A (tab name in session context) and all rows from staging sheet B (tab name in session context). **The MCP tool returns at most 50 rows per call — a single large range like `A1:I1000` is silently truncated to 50 rows and you will miss all findings after row 50.** Read each staging tab in batched increments: `{staging_tab_name}!A1:I50`, then `{staging_tab_name}!A51:I100`, `{staging_tab_name}!A101:I150`, continuing in 50-row increments until two consecutive batches return no non-empty rows. Collect all rows across all batches. Count the non-empty rows in each tab, excluding the header row (row 1).
...
- Leave column I (Status) blank for now — the researcher will review.
