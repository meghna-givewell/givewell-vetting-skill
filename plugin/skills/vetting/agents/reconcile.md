...
- **Confirmed**: both A and B caught it → keep the version with more complete Explanation and Recommended Fix. If A and B assigned different severities, **apply the severity decision tree from `reference/output-format.md`** to determine the correct severity:
  ...
  5. **After running the decision tree**: if either instance rated this finding High, apply the High-severity protection rule — retain High and note both ratings in column F (append to Explanation). The decision tree producing Medium does not override the protection rule. Apply the High-severity protection rule after running the decision tree, not before. Exception (FP-007): do NOT apply the high-severity protection rule when column H of the surviving finding is "No CE impact" — a confirmed zero-CE-impact finding must not be elevated beyond Low regardless of how many instances rated it High. The decision tree result is used only when neither instance rated this finding High and the protection rule does not apply.
  ...
  **Low + Legibility severity guard**: When the finding's Error Type (column E) is Legibility **and both A and B instances filed with blank column D**, leave column D (Severity) **blank** regardless of how the decision tree would otherwise classify Nature — these route to Publication Readiness. A blank column D from both instances is the definitive observable signal that the finding is publication-readiness scope; do not apply the Defect floor to Legibility findings that both instances agreed should be PR-routed. Medium and High Legibility findings write the severity in column D (Medium or High) and route to Findings.
  ...
  **Architectural scope**: The severity decision tree and high-severity protection rule above apply within this pair only. Cross-agent deduplication and severity comparison between different pairs (e.g., a Medium/H finding from formula-check-arithmetic vs. a High/D finding from formula-check-parameters for the same GBD vintage cell) is handled by the final-review-compaction agent via FORM-8 — not by this reconcile agent. Do not attempt to read or compare other agents' staging tabs.
...
- **B-only retained**: queue the row from stg-B for write to stg-rec-{pair}, resetting column I to blank — subject to the RECON-9 final check before actually writing. Do not write to the Findings or Publication Readiness sheets directly. Write all columns; do not add any meta-commentary to the Explanation.
  ...
  **A-only Medium finding calibration (RECON-16)**: When an A-only finding with severity **Medium** is being Retained (all three Won't Fix conditions were checked and failed — the finding was not marked Won't Fix), apply this additional calibration before writing the finding to stg-rec. Check whether Instance B demonstrably covered the same check area by examining: (a) B's AGENT_COMPLETE row column F from Step 0 — does it contain a check log entry for the relevant check category (e.g., "cross-geography outlier check [✓]") with no `[___]` placeholder, AND B's COVERAGE_ROWS (if present) includes the specific row of this finding's cell? OR (b) B filed at least one finding in the same sheet and row range (±10 rows), indicating B read and analyzed that area?

  If either condition (a) or (b) holds: downgrade this A-only finding from Medium to Low before writing it to stg-rec. Append to column F: "Instance B covered this area and found no Medium issue here — downgraded to Low/Judgment per RECON-16. Researcher to confirm if Medium severity is warranted." This prevents a single-instance Medium finding from reaching the final output at full confidence when the adversarial agent that reviewed the same area independently found nothing.

  **Do NOT apply RECON-16 to High-severity A-only findings** — the High-severity protection rule (see below) takes precedence. Do NOT apply RECON-16 when B's coverage of the cell cannot be determined — retain at Medium in that case (insufficient evidence to downgrade). Do NOT apply RECON-16 to A-only Low findings — no downgrade needed.
...
1. **B-only divergences retained after Step 4 investigation** — findings B caught that A missed, confirmed valid. Copy the row from stg-B verbatim, resetting column I to blank. Exception: if the B-only finding is Low severity and is a per-cell Low (column C contains a single cell reference rather than a grouped list of cells), regroup it into the appropriate standard Low category (per the grouping rule below and output-format.md) rather than writing it verbatim as a standalone per-cell Low.
