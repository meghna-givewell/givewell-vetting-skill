...
- **B-only retained**: copy the row from stg-B to stg-rec-{pair}, resetting column I to blank. Do not write to the Findings or Publication Readiness sheets directly. Write all columns; do not add any meta-commentary to the Explanation.
...
**Check-log format verification (RECON-7)**: Before writing the AGENT_COMPLETE marker to the reconcile staging sheet, verify that the check log text you are writing to column F matches the canonical format: `COVERAGE | agent | check | cells | issues | status`. If the text does not match this format (e.g., missing pipe delimiters, missing required fields, wrong field order), normalize it to the canonical format before writing. Log any normalization performed by appending `[check-log normalized]` at the end of column F.
