...
Exception: heads-up-intervention, heads-up-evidence, heads-up-epi, formula-check-structure, and sources may legitimately produce fewer than 3 findings depending on pub-readiness scope inclusion and model cleanliness — use judgment. If the sheet is a simple BOTEC and sources found 1 finding, that may be valid. If formula-check found 0 findings on a 100-row CEA, that is not plausible. The completion marker absence is a stronger signal than low finding count — always flag if the marker is absent.
...
Two findings **match** if they reference the same cell or overlapping row range (column C) AND describe the same underlying issue type (column E). Wording differences don't matter — "C48: GBD age group references 'All ages' row" and "C48: wrong age band in Busia column" match. Err toward treating findings as matching rather than distinct.
...
- **Confirmed**: both A and B caught it → keep the version with more complete Explanation and Recommended Fix. If A and B assigned different severities, **apply the severity decision tree from `reference/output-format.md`** to determine the correct severity:
  ...
  Do not note the severity comparison in the Explanation. Do not append any meta-commentary (e.g., "Confirmed by both independent agents", "Two independent agents assessed this at different severities") to the surviving row's Explanation. Exception: when the High-severity protection rule applies (Step 4), write the severity comparison in column I (Researcher judgment needed) as a note — e.g., 'Instance A: High. Instance B: Medium. High retained per protection rule.' Do not put this meta-commentary in column F (Explanation).
