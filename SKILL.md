...
>
> **Scope enforcement**: Only read, analyze, and file findings for sheets listed in `In-scope sheets`. Do not call `read_sheet_values`, `read_sheet_notes`, or `read_sheet_hyperlinks` on any sheet in `Out-of-scope sheets`. If a formula in an in-scope sheet references a cell in an out-of-scope sheet, note the cross-reference as context (e.g., "pulls from [SheetName] which is outside vet scope") but do not attempt to audit, validate, or characterize the referenced sheet's contents beyond what the formula reveals. Filing a finding about an out-of-scope sheet is not permitted — if the cross-reference itself raises a concern, file the finding against the in-scope cell that contains the reference.
>
